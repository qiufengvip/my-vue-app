<template>
  <el-container class="body">
    <el-header class="header">
      <!--        顶部导航  -->
      <MenuBoard></MenuBoard>
    </el-header>
    <el-main>
      <!--        主体-->
      <div class="body">
        <div class="main">
          <div class="left">
            <div class="left-main">
              <!--  展示 -->
              <div class="article-top">
                <div class="post-title">SpringCloud微服务框架入门与简单使用</div>
                <div class="label selected">
                  <div class="label-item">
                    <div>
                      <svg t="1656695778149" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="13323" width="200" height="200">
                        <path
                          d="M160.755264 64.708767c-52.871161 0-96.052636 43.177382-96.052636 96.010681L64.702628 384.970862c0 52.749388 30.578436 126.584 68.009953 163.941839l382.333802 382.327662c37.314861 37.399795 98.380658 37.399795 135.775336 0l280.4606-280.413528c37.352723-37.394678 37.352723-98.506525 0-135.859247L548.912701 132.635833c-37.357839-37.394678-111.109564-67.927065-163.941839-67.927065L160.755264 64.708767 160.755264 64.708767zM288.335964 384.184963c-52.913117 0-95.842859-42.930765-95.842859-95.842859 0-52.91721 42.929742-95.842859 95.842859-95.842859 52.916187 0 95.842859 42.929742 95.842859 95.842859C384.178823 341.253174 341.252151 384.184963 288.335964 384.184963L288.335964 384.184963zM288.335964 384.184963"
                          p-id="13324"
                        ></path>
                      </svg>
                    </div>
                    <div>SpringCloud</div>
                  </div>
                  <div class="label-item">
                    <div>
                      <svg t="1656695778149" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="13323" width="200" height="200">
                        <path
                          d="M160.755264 64.708767c-52.871161 0-96.052636 43.177382-96.052636 96.010681L64.702628 384.970862c0 52.749388 30.578436 126.584 68.009953 163.941839l382.333802 382.327662c37.314861 37.399795 98.380658 37.399795 135.775336 0l280.4606-280.413528c37.352723-37.394678 37.352723-98.506525 0-135.859247L548.912701 132.635833c-37.357839-37.394678-111.109564-67.927065-163.941839-67.927065L160.755264 64.708767 160.755264 64.708767zM288.335964 384.184963c-52.913117 0-95.842859-42.930765-95.842859-95.842859 0-52.91721 42.929742-95.842859 95.842859-95.842859 52.916187 0 95.842859 42.929742 95.842859 95.842859C384.178823 341.253174 341.252151 384.184963 288.335964 384.184963L288.335964 384.184963zM288.335964 384.184963"
                          p-id="13324"
                        ></path>
                      </svg>
                    </div>
                    <div>微服务框架</div>
                  </div>
                  <div class="label-item">
                    <div>
                      <svg t="1656695778149" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="13323" width="200" height="200">
                        <path
                          d="M160.755264 64.708767c-52.871161 0-96.052636 43.177382-96.052636 96.010681L64.702628 384.970862c0 52.749388 30.578436 126.584 68.009953 163.941839l382.333802 382.327662c37.314861 37.399795 98.380658 37.399795 135.775336 0l280.4606-280.413528c37.352723-37.394678 37.352723-98.506525 0-135.859247L548.912701 132.635833c-37.357839-37.394678-111.109564-67.927065-163.941839-67.927065L160.755264 64.708767 160.755264 64.708767zM288.335964 384.184963c-52.913117 0-95.842859-42.930765-95.842859-95.842859 0-52.91721 42.929742-95.842859 95.842859-95.842859 52.916187 0 95.842859 42.929742 95.842859 95.842859C384.178823 341.253174 341.252151 384.184963 288.335964 384.184963L288.335964 384.184963zM288.335964 384.184963"
                          p-id="13324"
                        ></path>
                      </svg>
                    </div>
                    <div>微服务介绍</div>
                  </div>
                </div>
                <div class="property-s selected">
                  <div class="property">
                    <div>
                      <svg t="1656612504292" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5903" width="200" height="200">
                        <path
                          d="M435.190784 0 51.198976 0c-28.300288 0-51.198976 22.900736-51.198976 51.149824l0 383.543296c0 28.197888 22.898688 51.098624 51.198976 51.098624l383.991808 0c28.250112 0 51.198976-22.900736 51.198976-51.098624L486.38976 51.149824C486.38976 22.900736 463.440896 0 435.190784 0zM435.190784 536.891392 51.198976 536.891392c-28.300288 0-51.198976 22.897664-51.198976 51.195904l0 383.49312c0 28.20096 22.898688 51.098624 51.198976 51.098624l383.991808 0c28.250112 0 51.198976-22.897664 51.198976-51.098624L486.38976 588.087296C486.38976 559.789056 463.440896 536.891392 435.190784 536.891392zM907.880448 870.583296c-1.256448-1.256448-2.610176-2.353152-4.035584-3.294208 27.150336-34.69312 43.334656-78.357504 43.334656-125.802496 0-112.9984-91.6992-204.5952-204.795904-204.5952-113.097728 0-204.796928 91.5968-204.796928 204.5952 0 112.948224 91.6992 204.493824 204.796928 204.493824 47.514624 0 91.24864-16.162816 125.997056-43.278336 0.941056 1.426432 2.040832 2.778112 3.299328 4.031488l90.500096 90.396672c9.998336 10.002432 26.19904 10.002432 36.201472 0 9.998336-9.94816 9.998336-26.14784 0-36.150272L907.880448 870.583296zM742.383616 894.880768c-84.84864 0-153.595904-68.696064-153.595904-153.3952 0-84.750336 68.747264-153.399296 153.595904-153.399296 84.847616 0 153.597952 68.64896 153.597952 153.399296C895.981568 826.184704 827.232256 894.880768 742.383616 894.880768zM972.77952 0 588.787712 0c-28.300288 0-51.201024 22.900736-51.201024 51.149824l0 383.543296c0 28.197888 22.900736 51.098624 51.201024 51.098624l383.991808 0c28.249088 0 51.196928-22.900736 51.196928-51.098624L1023.976448 51.149824C1023.976448 22.900736 1001.028608 0 972.77952 0z"
                          p-id="5904"
                          fill="#e0620d"
                        ></path>
                      </svg>
                    </div>
                    <div>项目介绍</div>
                  </div>
                  <div class="property">
                    <div>
                      <svg t="1656612564413" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="9421" width="200" height="200">
                        <path
                          d="M512 872.838095c-24.380952 0-46.32381-4.87619-68.266667-17.066666l-365.714285-190.171429-51.2 26.819048c-34.133333 17.066667-34.133333 68.266667 0 85.333333l451.047619 236.495238c9.752381 4.87619 21.942857 7.314286 34.133333 7.314286s24.380952-2.438095 34.133333-7.314286l451.047619-236.495238c34.133333-17.066667 34.133333-68.266667 0-85.333333l-51.2-26.819048-365.714285 190.171429c-21.942857 12.190476-43.885714 17.066667-68.266667 17.066666z"
                          fill="#8F65FF"
                          p-id="9422"
                        ></path>
                        <path
                          d="M997.180952 468.114286l-51.2-26.819048-134.095238 70.704762-80.457143 41.447619-151.161904 80.457143c-21.942857 9.752381-43.885714 17.066667-68.266667 17.066667s-46.32381-4.87619-68.266667-17.066667L292.571429 553.447619 212.114286 512l-134.095238-70.704762-51.2 26.819048c-34.133333 17.066667-34.133333 68.266667 0 85.333333l51.2 26.819048 78.019047 41.447619 319.390476 168.228571c9.752381 4.87619 21.942857 7.314286 34.133334 7.314286s24.380952-2.438095 34.133333-7.314286l319.390476-168.228571 78.019048-41.447619 51.2-26.819048c39.009524-17.066667 39.009524-65.828571 4.87619-85.333333z"
                          fill="#C7B2FF"
                          p-id="9423"
                        ></path>
                        <path
                          d="M512 799.695238c-12.190476 0-24.380952-2.438095-34.133333-7.314286L158.47619 624.152381l-78.019047 41.447619 365.714286 190.171429c21.942857 9.752381 43.885714 17.066667 68.266666 17.066666s46.32381-4.87619 68.266667-17.066666l365.714286-190.171429-78.019048-41.447619-319.390476 168.228571c-14.628571 4.87619-26.819048 7.314286-39.009524 7.314286z"
                          fill="#FFFFFF"
                          p-id="9424"
                        ></path>
                        <path
                          d="M997.180952 246.247619L546.133333 9.752381c-4.87619-2.438095-9.752381-4.87619-17.066666-7.314286-4.87619-2.438095-12.190476-2.438095-17.066667-2.438095s-12.190476 0-17.066667 2.438095-12.190476 2.438095-17.066666 7.314286L26.819048 246.247619c-34.133333 17.066667-34.133333 68.266667 0 85.333333l51.2 26.819048 78.019047 41.447619 134.095238 70.704762 78.019048 41.447619 107.27619 56.07619c9.752381 4.87619 21.942857 7.314286 34.133334 7.314286s24.380952-2.438095 34.133333-7.314286l107.276191-56.07619 78.019047-41.447619 134.095238-70.704762 78.019048-41.447619 51.2-26.819048c39.009524-17.066667 39.009524-68.266667 4.87619-85.333333z"
                          fill="#8F65FF"
                          p-id="9425"
                        ></path>
                        <path
                          d="M529.066667 4.87619c-12.190476-2.438095-24.380952-2.438095-34.133334 0 4.87619-2.438095 12.190476-2.438095 17.066667-2.438095 4.87619 0 12.190476 0 17.066667 2.438095z"
                          fill="#FFFFFF"
                          p-id="9426"
                        ></path>
                      </svg>
                    </div>
                    <div>字数: 1.5k</div>
                  </div>
                  <div class="property">
                    <div>
                      <svg t="1656612599760" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="12823" width="200" height="200">
                        <path
                          d="M766.208 958.336H269.696c-112.768 0-204.032-91.392-204.032-204.032V257.792c0-112.64 91.264-204.032 204.032-204.032h496.512c112.64 0 204.032 91.392 204.032 204.032v496.512c0 112.768-91.392 204.032-204.032 204.032z"
                          fill="#FF9429"
                          p-id="12824"
                        ></path>
                        <path
                          d="M705.408 743.04c-1.664-10.624-3.84-20.992-6.272-30.976-1.152-4.736-2.688-9.216-4.096-13.824-1.792-6.016-3.456-12.032-5.632-17.792-1.92-5.248-3.968-10.112-6.144-15.232-2.048-4.736-3.968-9.6-6.144-14.208-2.432-5.12-5.12-10.112-7.808-14.976-2.176-3.968-4.48-7.936-6.784-11.776-2.944-4.864-6.144-9.472-9.344-14.08-2.432-3.456-4.864-6.656-7.424-9.856-3.456-4.352-7.04-8.704-10.752-12.8-2.688-2.816-5.376-5.504-8.064-8.192-3.84-3.84-7.808-7.552-11.904-11.008-2.944-2.432-6.016-4.48-8.96-6.656-4.224-3.072-8.32-6.272-12.672-8.832-0.256-0.128-0.512-0.384-0.768-0.512-13.312-7.936-21.376-22.016-21.376-37.504v-17.536c0-15.488 8.192-29.568 21.376-37.504 0.256-0.128 0.512-0.384 0.768-0.512 4.352-2.688 8.448-5.76 12.672-8.832 3.072-2.176 6.016-4.224 8.96-6.656 4.096-3.456 7.936-7.168 11.904-11.008 2.688-2.688 5.504-5.376 8.192-8.192 3.712-4.096 7.296-8.32 10.752-12.672 2.56-3.2 4.992-6.528 7.424-9.856 3.2-4.48 6.4-9.216 9.344-14.08 2.432-3.84 4.608-7.808 6.784-11.776 2.688-4.864 5.376-9.856 7.808-14.976 2.176-4.608 4.224-9.344 6.144-14.208 2.048-4.992 4.224-9.984 6.144-15.232 2.048-5.76 3.84-11.776 5.632-17.792 1.408-4.608 2.944-9.088 4.096-13.824 2.56-10.112 4.608-20.48 6.272-30.976 0.128-0.896 0.384-1.664 0.512-2.56 4.096-26.752-16.384-51.072-43.52-51.072H373.376c-27.136 0-47.616 24.32-43.52 51.072 0.128 0.896 0.384 1.664 0.512 2.56 1.664 10.624 3.84 20.992 6.272 31.104 1.152 4.736 2.688 9.216 4.096 13.824 1.792 6.016 3.456 12.032 5.632 17.792 1.92 5.248 3.968 10.112 6.144 15.232 2.048 4.736 3.968 9.6 6.144 14.208 2.432 5.12 5.12 10.112 7.808 14.976 2.176 3.968 4.48 7.936 6.784 11.776 2.944 4.864 6.144 9.472 9.344 14.08 2.432 3.456 4.864 6.656 7.424 9.856 3.456 4.352 7.04 8.704 10.752 12.672 2.688 2.816 5.376 5.504 8.192 8.192 3.84 3.84 7.808 7.552 11.904 11.008 2.944 2.432 6.016 4.48 8.96 6.656 4.224 3.072 8.32 6.272 12.672 8.832 0.256 0.128 0.512 0.384 0.768 0.512 13.312 7.936 21.376 22.016 21.376 37.504v17.536c0 15.488-8.192 29.568-21.376 37.504-0.256 0.128-0.512 0.384-0.768 0.512-4.352 2.688-8.448 5.76-12.672 8.832-2.944 2.176-6.016 4.224-8.96 6.656-4.096 3.456-7.936 7.168-11.904 11.008-2.688 2.688-5.504 5.248-8.064 8.192-3.712 4.096-7.296 8.32-10.752 12.8-2.56 3.2-4.992 6.528-7.424 9.856-3.2 4.608-6.4 9.216-9.344 14.08-2.304 3.84-4.608 7.808-6.784 11.776-2.688 4.864-5.376 9.856-7.808 14.976-2.176 4.608-4.224 9.344-6.144 14.208-2.048 4.992-4.224 9.984-6.144 15.232-2.048 5.76-3.84 11.776-5.632 17.792-1.408 4.608-2.944 9.088-4.096 13.824-2.56 10.112-4.608 20.48-6.272 31.104-0.128 0.896-0.384 1.664-0.512 2.56-4.096 26.752 16.384 51.072 43.52 51.072h289.024c27.136 0 47.616-24.32 43.52-51.072-0.256-1.024-0.512-1.92-0.64-2.816z"
                          fill="#FFFFFF"
                          p-id="12825"
                        ></path>
                        <path
                          d="M415.872 755.712h204.16c7.68 0 11.264-9.728 5.248-14.592l-93.952-73.216c-1.92-1.536-2.944-3.84-2.944-6.4V425.856c0-2.304 1.024-4.48 2.688-6.016l55.936-45.952c5.504-5.12 1.92-14.336-5.632-14.336H454.4c-7.552 0-11.136 9.216-5.632 14.336l55.296 45.696c1.664 1.536 2.688 3.712 2.688 6.016v235.648c0 2.432-1.152 4.736-2.944 6.4L410.496 741.12c-5.888 4.992-2.432 14.592 5.376 14.592z"
                          fill="#FF9429"
                          p-id="12826"
                        ></path>
                      </svg>
                    </div>
                    <div>阅读时长: 4分钟</div>
                  </div>
                  <div class="property selected">
                    <div>
                      <svg t="1656612443296" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1684" width="200" height="200">
                        <path
                          d="M896 85.333333C299.008 85.333333 184.661333 605.013333 137.344 882.688l-2.346667 13.866667a36.138667 36.138667 0 0 0 35.626667 42.112h8.917333a42.666667 42.666667 0 0 0 41.344-32.128c1.877333-7.381333 3.584-13.525333 5.034667-18.304 34.218667-110.378667 101.12-171.776 200.746667-184.234667 170.666667-21.333333 298.666667-170.666667 341.333333-298.666667l-64-42.666666 42.666667-42.666667c42.666667-42.666667 85.504-106.666667 149.333333-234.666667z"
                          fill="#F64E54"
                          p-id="1685"
                        ></path>
                      </svg>
                    </div>
                    <div>2020-06-30 12:00:00</div>
                  </div>
                  <div class="property">
                    <div>
                      <svg t="1656612479158" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4881" width="200" height="200">
                        <path d="M512 512m-512 0a512 512 0 1 0 1024 0 512 512 0 1 0-1024 0Z" fill="#1296db" p-id="4882" data-spm-anchor-id="a313x.7781069.0.i2" class="selected"></path>
                        <path
                          d="M464.804 619.285l18.761-43.15s16.468-42.525-40.649-32.311l-160.303 40.44s-14.513 4.15-20.94 14.854a206.407 206.407 0 0 1 15.828-0.618c90.542 0 165.369 58.964 177.322 135.498 81.825 17.702 277.417 33.239 326.836-205.182-0.001 0-74.211 175.312-316.855 90.469zM560.693 400.406L541.1 442.098s-17.719 40.857 39.606 32.311l160.72-35.646s36.689-9.381 20.429-39.399l-87.552-146.336s-27.309-24.181-45.443 4.586l-19.386 41.691c-0.21 0-296.634-112.567-367.093 181.982 0 0 78.588-169.058 318.312-80.881z"
                          fill="#FFFFFF"
                          p-id="4883"
                        ></path>
                        <path
                          d="M277.5 598.5c-5.335 0-10.611 0.217-15.828 0.618-3.762 6.264-4.757 14.769 1.554 26.005l91.721 148.213s27.516 24.18 45.235-6.046l18.344-43.15s13.884 5.011 36.295 9.858C442.869 657.464 368.042 598.5 277.5 598.5z"
                          fill="#FFFFFF"
                          opacity=".4"
                          p-id="4884"
                        ></path>
                      </svg>
                    </div>
                    <div>2022-05-08 15:10:24</div>
                  </div>
                </div>
                <div class="middle-t">
                  <div class="comment-main-reply">
                    <div class="t-left"><img src="http://file.luanshiliunian.cn/11111_1637034086344.jpg" alt="a" /></div>
                    <div class="t-right">
                      <div class="right-top-main">
                        <div class="right-top">
                          <div class="right-top-name">秋枫</div>
                          <div class="right-top-id">
                            <svg t="1643960221131" class="right-top-tag" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2785">
                              <path
                                d="M505.8 201.7c-17 0-30.8-13.8-30.8-30.8s13.8-30.8 30.8-30.8 30.8 13.8 30.8 30.8c0.1 17-13.8 30.8-30.8 30.8z m0-46.5c-8.6 0-15.7 7-15.7 15.7 0 8.6 7 15.7 15.7 15.7s15.7-7 15.7-15.7c0-8.6-7-15.7-15.7-15.7z"
                                fill="#47B7F8"
                                p-id="2786"
                              ></path>
                              <path
                                d="M677 170h-13.5v-13.5c0-3.6-2.9-6.5-6.5-6.5s-6.5 2.9-6.5 6.5V170H637c-3.6 0-6.5 2.9-6.5 6.5s2.9 6.5 6.5 6.5h13.5v13.5c0 3.6 2.9 6.5 6.5 6.5s6.5-2.9 6.5-6.5V183H677c3.6 0 6.5-2.9 6.5-6.5s-2.9-6.5-6.5-6.5z"
                                fill="#F7E42F"
                                p-id="2787"
                              ></path>
                              <path
                                d="M938.7 436.8H923v-15.7c0-4.2-3.4-7.6-7.6-7.6s-7.6 3.4-7.6 7.6v15.7h-15.7c-4.2 0-7.6 3.4-7.6 7.6 0 4.2 3.4 7.6 7.6 7.6h15.7v15.7c0 4.2 3.4 7.6 7.6 7.6s7.6-3.4 7.6-7.6V452h15.7c4.2 0 7.6-3.4 7.6-7.6 0-4.3-3.4-7.6-7.6-7.6z"
                                fill="#F8B62D"
                                p-id="2788"
                              ></path>
                              <path d="M227.8 208.9m-11.4 0a11.4 11.4 0 1 0 22.8 0 11.4 11.4 0 1 0-22.8 0Z" fill="#F8B62D" p-id="2789"></path>
                              <path d="M781.3 230.9m-15.2 0a15.2 15.2 0 1 0 30.4 0 15.2 15.2 0 1 0-30.4 0Z" fill="#F8B62D" p-id="2790"></path>
                              <path d="M115.2 413.5m-15.2 0a15.2 15.2 0 1 0 30.4 0 15.2 15.2 0 1 0-30.4 0Z" fill="#F7E42F" p-id="2791"></path>
                              <path
                                d="M132.6 651.4c-17 0-30.8-13.8-30.8-30.8s13.8-30.8 30.8-30.8 30.8 13.8 30.8 30.8-13.9 30.8-30.8 30.8z m0-46.4c-8.6 0-15.7 7-15.7 15.7 0 8.6 7 15.7 15.7 15.7s15.7-7 15.7-15.7c-0.1-8.7-7.1-15.7-15.7-15.7z"
                                fill="#F8B62D"
                                p-id="2792"
                              ></path>
                              <path
                                d="M318.6 68.3c-1.6-1.6-4.3-1.6-5.9 0-0.8 0.8-1.2 1.9-1.2 3s0.4 2.2 1.2 3l13.5 13.5c0.8 0.8 1.8 1.2 3 1.2s2.2-0.4 3-1.2c1.6-1.6 1.6-4.3 0-5.9l-13.6-13.6zM354.1 103.9c-0.8-0.8-1.8-1.2-3-1.2-1.1 0-2.2 0.4-3 1.2-1.6 1.6-1.6 4.3 0 5.9l13.5 13.5c0.8 0.8 1.8 1.2 3 1.2 1.1 0 2.2-0.4 3-1.2 0.8-0.8 1.2-1.8 1.2-3 0-1.1-0.4-2.2-1.2-3l-13.5-13.4zM351.2 89c1.1 0 2.2-0.4 3-1.2l13.5-13.5c0.8-0.8 1.2-1.8 1.2-3 0-1.1-0.4-2.2-1.2-3-1.6-1.6-4.3-1.6-5.9 0l-13.5 13.5c-1.6 1.6-1.6 4.3 0 5.9 0.7 0.8 1.7 1.3 2.9 1.3zM326.1 103.9l-13.5 13.5c-0.8 0.8-1.2 1.9-1.2 3s0.4 2.2 1.2 3c0.8 0.8 1.8 1.2 3 1.2 1.1 0 2.2-0.4 3-1.2l13.5-13.5c1.6-1.6 1.6-4.3 0-5.9-1.6-1.7-4.4-1.7-6-0.1zM328.7 95.8c0-2.3-1.9-4.2-4.2-4.2h-19c-2.3 0-4.2 1.9-4.2 4.2s1.9 4.2 4.2 4.2h19c2.3 0 4.2-1.9 4.2-4.2zM374.8 91.6h-19c-2.3 0-4.2 1.9-4.2 4.2s1.9 4.2 4.2 4.2h19c2.3 0 4.2-1.9 4.2-4.2s-1.9-4.2-4.2-4.2zM340.1 57c-2.3 0-4.2 1.9-4.2 4.2v19c0 2.3 1.9 4.2 4.2 4.2 2.3 0 4.2-1.9 4.2-4.2v-19c0-2.4-1.9-4.2-4.2-4.2zM340.1 107.2c-2.3 0-4.2 1.9-4.2 4.2v19c0 2.3 1.9 4.2 4.2 4.2 2.3 0 4.2-1.9 4.2-4.2v-19c0-2.3-1.9-4.2-4.2-4.2z"
                                fill="#FAEE00"
                                p-id="2793"
                              ></path>
                              <path
                                d="M831.1 624c-1.6-1.6-4.3-1.6-5.9 0-0.8 0.8-1.2 1.9-1.2 3s0.4 2.2 1.2 3l13.5 13.5c0.8 0.8 1.8 1.2 3 1.2s2.2-0.4 3-1.2c1.6-1.6 1.6-4.3 0-5.9L831.1 624zM866.7 659.5c-0.8-0.8-1.8-1.2-3-1.2-1.1 0-2.2 0.4-3 1.2-1.6 1.6-1.6 4.3 0 5.9l13.5 13.5c0.8 0.8 1.8 1.2 3 1.2 1.1 0 2.2-0.4 3-1.2 0.8-0.8 1.2-1.8 1.2-3s-0.4-2.2-1.2-3l-13.5-13.4zM863.7 644.6c1.1 0 2.2-0.4 3-1.2l13.5-13.5c0.8-0.8 1.2-1.8 1.2-3s-0.4-2.2-1.2-3c-1.6-1.6-4.3-1.6-5.9 0l-13.5 13.5c-1.6 1.6-1.6 4.3 0 5.9 0.7 0.9 1.8 1.3 2.9 1.3zM838.7 659.5L825.2 673c-0.8 0.8-1.2 1.9-1.2 3s0.4 2.2 1.2 3c0.8 0.8 1.8 1.2 3 1.2 1.1 0 2.2-0.4 3-1.2l13.5-13.5c1.6-1.6 1.6-4.3 0-5.9-1.7-1.7-4.4-1.7-6-0.1zM841.3 651.4c0-2.3-1.9-4.2-4.2-4.2h-19c-2.3 0-4.2 1.9-4.2 4.2 0 2.3 1.9 4.2 4.2 4.2h19c2.3 0 4.2-1.8 4.2-4.2zM887.3 647.2h-19c-2.3 0-4.2 1.9-4.2 4.2 0 2.3 1.9 4.2 4.2 4.2h19c2.3 0 4.2-1.9 4.2-4.2 0-2.3-1.9-4.2-4.2-4.2zM852.7 612.6c-2.3 0-4.2 1.9-4.2 4.2v19c0 2.3 1.9 4.2 4.2 4.2 2.3 0 4.2-1.9 4.2-4.2v-19c0-2.3-1.9-4.2-4.2-4.2zM852.7 662.8c-2.3 0-4.2 1.9-4.2 4.2v19c0 2.3 1.9 4.2 4.2 4.2 2.3 0 4.2-1.9 4.2-4.2v-19c0-2.3-1.9-4.2-4.2-4.2z"
                                fill="#F8B62D"
                                p-id="2794"
                              ></path>
                              <path
                                d="M873.1 919.4H846c-6.3 0-11.4-5.1-11.4-11.4 0-6.3 5.1-11.4 11.4-11.4h27.1c6.3 0 11.4 5.1 11.4 11.4 0 6.3-5.1 11.4-11.4 11.4zM149.9 919.4h-34.6c-6.3 0-11.4-5.1-11.4-11.4 0-6.3 5.1-11.4 11.4-11.4h34.6c6.3 0 11.4 5.1 11.4 11.4-0.1 6.3-5.1 11.4-11.4 11.4zM812.6 919.4H188.1c-6.3 0-11.4-5.1-11.4-11.4 0-6.3 5.1-11.4 11.4-11.4h624.5c6.3 0 11.4 5.1 11.4 11.4 0 6.3-5.1 11.4-11.4 11.4z"
                                fill="#3E3A39"
                                p-id="2795"
                              ></path>
                              <path
                                d="M481.7 879.3L178.1 508.9c-7.6-9.3-8.6-22.3-2.5-32.6l106.2-180c5.3-8.9 15-14.4 25.5-14.4H720c10.7 0 20.6 5.7 25.8 15l100.4 179.6c5.7 10.3 4.6 22.9-2.9 32l-304 370.8c-14.8 18.1-42.8 18.1-57.6 0z"
                                fill="#FCEE21"
                                p-id="2796"
                              ></path>
                              <path
                                d="M745.7 296.9c-5.2-9.2-15.1-15-25.8-15H503.4v610.2c12.9 2.4 26.8-1.8 35.8-12.8l303.9-370.8c7.5-9.1 8.6-21.8 2.9-32L745.7 296.9z"
                                fill="#F8B62D"
                                p-id="2797"
                              ></path>
                              <path
                                d="M501.1 904.2c-15 0-29.1-6.7-38.5-18.4L149.9 497.3c-10.7-13.3-12.2-32.2-3.6-47l46.2-79.8c3.1-5.4 10.1-7.3 15.5-4.1 5.4 3.1 7.3 10.1 4.1 15.5L166 461.7c-3.9 6.7-3.2 15.3 1.6 21.4l312.7 388.5c5.1 6.3 12.7 10 20.8 10 8.1 0 15.7-3.6 20.8-10l313-388.9c4.8-6 5.6-14.2 1.8-21L733.4 273.3c-3.3-6.1-9.7-9.8-16.6-9.8H291.7c-6.7 0-13 3.6-16.4 9.5l-8.5 14.7c-3.1 5.4-10.1 7.3-15.5 4.1-5.4-3.1-7.3-10.1-4.1-15.5l8.5-14.7c7.4-12.8 21.3-20.8 36.1-20.8h425.1c15.2 0 29.2 8.3 36.5 21.6l103.4 188.3c8.2 14.9 6.6 33-4.1 46.2l-313 388.9c-9.6 11.7-23.6 18.4-38.6 18.4z"
                                fill="#3E3A39"
                                p-id="2798"
                              ></path>
                              <path
                                d="M223.2 346.1c-1.9 0-3.9-0.5-5.7-1.5-5.4-3.1-7.3-10.1-4.1-15.5l11.4-19.6c3.1-5.4 10.1-7.3 15.5-4.1 5.4 3.1 7.3 10.1 4.1 15.5L233 340.4c-2.1 3.7-5.9 5.7-9.8 5.7z"
                                fill="#3E3A39"
                                p-id="2799"
                              ></path>
                              <path d="M502.5 886.9L630.4 512" fill="#FCEE21" p-id="2800"></path>
                              <path
                                d="M502.5 898.3c-1.2 0-2.5-0.2-3.7-0.6-5.9-2-9.1-8.5-7.1-14.4l127.9-375c2-5.9 8.5-9.1 14.4-7.1s9.1 8.5 7.1 14.4l-127.9 375c-1.5 4.7-5.9 7.7-10.7 7.7z"
                                fill="#3E3A39"
                                p-id="2801"
                              ></path>
                              <path d="M501.1 886.9L363.7 502.4" fill="#FCEE21" p-id="2802"></path>
                              <path
                                d="M501.1 898.3c-4.7 0-9-2.9-10.7-7.5L353 506.3c-2.1-5.9 1-12.4 6.9-14.5 5.9-2.1 12.4 1 14.5 6.9l137.4 384.5c2.1 5.9-1 12.4-6.9 14.5-1.3 0.4-2.5 0.6-3.8 0.6z"
                                fill="#3E3A39"
                                p-id="2803"
                              ></path>
                              <path
                                d="M345 441.6c-2.7 0-5.5-1-7.7-3-4.6-4.2-5-11.4-0.7-16.1l156-170.6c2.1-2.3 5.1-3.7 8.3-3.7h0.1c3.1 0 6.1 1.3 8.3 3.6L670 422.5c4.3 4.6 4.1 11.8-0.5 16.1-4.6 4.3-11.8 4.1-16.1-0.5L501.2 276.4 353.4 437.9c-2.2 2.5-5.3 3.7-8.4 3.7z"
                                fill="#3E3A39"
                                p-id="2804"
                              ></path>
                              <path
                                d="M786.6 467.7c0 19.2-15.7 34.8-34.8 34.8H285.6c-19.2 0-34.8-15.7-34.8-34.8 0-19.2 15.7-34.8 34.8-34.8h466.2c19.2 0 34.8 15.6 34.8 34.8z"
                                fill="#FFFFFF"
                                p-id="2805"
                              ></path>
                              <path
                                d="M751.8 513.9h-501c-25.5 0-46.2-20.7-46.2-46.2 0-25.5 20.7-46.2 46.2-46.2h501.1c25.5 0 46.2 20.7 46.2 46.2-0.1 25.5-20.8 46.2-46.3 46.2z m-501-69.7c-12.9 0-23.5 10.5-23.5 23.5 0 12.9 10.5 23.5 23.5 23.5h501.1c12.9 0 23.5-10.5 23.5-23.5 0-12.9-10.5-23.5-23.5-23.5H250.8z"
                                fill="#3E3A39"
                                p-id="2806"
                              ></path>
                              <path
                                d="M275.8 387.5c-1.8 0-3.7-0.4-5.4-1.4-5.5-3-7.6-9.9-4.6-15.4l28.3-52.2c5-9.3 14.7-15.1 25.3-15.1H345c6.3 0 11.4 5.1 11.4 11.4s-5.1 11.4-11.4 11.4h-25.6c-2.2 0-4.2 1.2-5.3 3.1l-28.3 52.2c-2 3.9-5.9 6-10 6z"
                                fill="#FFFFFF"
                                p-id="2807"
                              ></path>
                            </svg>
                            <div class="right-top-grade">普通会员</div>
                          </div>
                        </div>
                      </div>
                      <div class="right-info">
                        <div class="right-info-time">让世界变得更美好</div>
                      </div>
                    </div>
                  </div>
                  <div class="property-s selected middle-p">
                    <div class="p-item">
                      <div>
                        <svg t="1656687868200" class="icon3" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="9015" width="200" height="200">
                          <path
                            d="M102.4 197.973333m27.306667 0l764.586666 0q27.306667 0 27.306667 27.306667l0 641.706667q0 27.306667-27.306667 27.306666l-764.586666 0q-27.306667 0-27.306667-27.306666l0-641.706667q0-27.306667 27.306667-27.306667Z"
                            fill="#FFA382"
                            p-id="9016"
                          ></path>
                          <path
                            d="M850.363733 197.973333C889.705813 197.973333 921.6 229.94944 921.6 269.39392v553.478827c0 39.44448-31.894187 71.420587-71.236267 71.420586h-676.727466C134.294187 894.293333 102.4 862.317227 102.4 822.872747v-35.703467h641.112747c38.147413 0 69.290667-30.071467 71.14752-67.857067l0.088746-3.56352V197.973333h35.61472z"
                            fill="#FB8E67"
                            p-id="9017"
                          ></path>
                          <path
                            d="M850.827947 177.493333H173.172053C119.002453 177.493333 75.093333 221.115733 75.093333 274.937173v549.218987C75.093333 877.9776 119.002453 921.6 173.172053 921.6h677.655894c54.1696 0 98.07872-43.6224 98.07872-97.44384V274.937173C948.906667 221.115733 904.997547 177.493333 850.827947 177.493333z m-677.655894 53.152427h677.655894c24.61696 0 44.578133 19.831467 44.578133 44.291413v549.218987c0 24.459947-19.961173 44.291413-44.578133 44.291413H173.172053c-24.61696 0-44.578133-19.831467-44.578133-44.291413V274.937173c0-24.459947 19.961173-44.291413 44.578133-44.291413z"
                            fill="#7B7BA3"
                            p-id="9018"
                          ></path>
                          <path
                            d="M329.8304 137.570987L498.346667 214.289067V873.813333L268.581547 769.201493A27.306667 27.306667 0 0 1 252.586667 744.352427V187.26912a54.613333 54.613333 0 0 1 77.243733-49.698133z"
                            fill="#FFA382"
                            p-id="9019"
                          ></path>
                          <path
                            d="M336.110933 102.4C282.43968 102.4 238.933333 146.152107 238.933333 200.116907v526.72512c0 37.983573 21.886293 72.51968 56.1152 88.569173l206.554454 96.836267c17.57184 8.23296 37.70368-4.662613 37.70368-24.159574V204.53376a26.658133 26.658133 0 0 0-15.305387-24.14592L377.173333 111.547733A96.733867 96.733867 0 0 0 336.110933 102.4z m3.857067 53.47328c5.12 0.45056 10.130773 1.795413 14.813867 3.986773l131.515733 61.6448v624.76288L317.446827 767.09888a44.434773 44.434773 0 0 1-25.504427-40.256853V200.116907c0-24.528213 19.776853-44.414293 44.168533-44.414294l3.857067 0.170667z"
                            fill="#7B7BA3"
                            p-id="9020"
                          ></path>
                          <path d="M607.573333 529.066667a98.986667 95.573333 90 1 0 191.146667 0 98.986667 95.573333 90 1 0-191.146667 0Z" fill="#FFEF8E" p-id="9021"></path>
                          <path
                            d="M758.196907 450.553173C790.295893 467.00544 812.373333 501.295787 812.373333 540.93824 812.373333 596.60288 768.873813 641.706667 715.209387 641.706667c-38.229333 0-71.304533-22.89664-87.16288-56.183467 5.38624 0.805547 10.89536 1.221973 16.493226 1.221973 63.419733 0 114.83136-53.316267 114.83136-119.084373 0-5.290667-0.32768-10.50624-0.976213-15.612587z"
                            fill="#F4DD55"
                            p-id="9022"
                          ></path>
                          <path
                            d="M703.146667 402.773333C635.282773 402.773333 580.266667 459.318613 580.266667 529.066667S635.282773 655.36 703.146667 655.36s122.88-56.54528 122.88-126.293333S771.01056 402.773333 703.146667 402.773333z m0 54.12864c38.775467 0 70.219093 32.310613 70.219093 72.164694S741.922133 601.23136 703.146667 601.23136c-38.775467 0-70.219093-32.310613-70.219094-72.164693S664.3712 456.901973 703.146667 456.901973z"
                            fill="#7B7BA3"
                            p-id="9023"
                          ></path>
                        </svg>
                      </div>
                      <div>469788</div>
                    </div>
                    <div class="p-item">
                      <div>
                        <svg t="1656687586445" class="icon3" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5858" width="200" height="200">
                          <path
                            d="M617.124 221.773h216.33c46.153 77.632 34.733 47.996 34.733 102.145l1.222 300.283c0 54.147-2.021 76.737-43.955 104.179H486.277l-57.332 10.007-145.402 120.719 13.382-114.904 6.127-12.256c-54.148 0-74.232-47.471-74.232-101.619V319.824c0-54.148 43.879-98.051 98.051-98.051h290.253z"
                            fill="#31D3FF"
                            p-id="5859"
                          ></path>
                          <path
                            d="M162.607 468.936c-13.537 0-24.513-10.964-24.513-24.514v-59.223c0-13.548 10.976-24.513 24.513-24.513s24.513 10.964 24.513 24.513v59.223c0 13.55-10.976 24.514-24.513 24.514z"
                            fill="#213847"
                            p-id="5860"
                          ></path>
                          <path
                            d="M283.543 883.619c-3.28 0-6.583-0.646-9.707-2.011a24.507 24.507 0 0 1-14.806-22.693l0.802-106.022c-67.207-0.456-121.738-55.25-121.738-122.565v-120.53c0-13.549 10.976-24.512 24.513-24.512s24.513 10.963 24.513 24.512v120.529c0 40.552 32.987 73.539 73.539 73.539h23.867a24.506 24.506 0 0 1 17.403 7.254 24.5 24.5 0 0 1 7.11 17.45l-0.55 72.725 94.652-90.631a24.542 24.542 0 0 1 16.949-6.798h347.176c40.553 0 73.539-32.987 73.539-73.539V319.824c0-40.551-32.986-73.539-73.539-73.539H260.658c-40.551 0-73.539 32.987-73.539 73.539 0 13.549-10.976 24.513-24.513 24.513s-24.513-10.963-24.513-24.513c0-67.578 54.986-122.564 122.564-122.564h506.605c67.578 0 122.565 54.986 122.565 122.564v310.503c0 67.579-54.987 122.565-122.565 122.565H429.926L300.492 876.821a24.52 24.52 0 0 1-16.949 6.798z"
                            fill="#213847"
                            p-id="5861"
                          ></path>
                          <path
                            d="M755.009 447.703h-484.32c-13.537 0-24.513-10.964-24.513-24.513 0-13.549 10.976-24.513 24.513-24.513h484.32c13.537 0 24.512 10.963 24.512 24.513 0 13.549-10.975 24.513-24.512 24.513zM640.607 562.894H270.665c-13.538 0-24.513-10.964-24.513-24.512 0-13.55 10.976-24.514 24.513-24.514h369.942c13.537 0 24.512 10.964 24.512 24.514 0 13.548-10.974 24.512-24.512 24.512z"
                            fill="#213847"
                            p-id="5862"
                          ></path>
                        </svg>
                      </div>
                      <div>25</div>
                    </div>
                    <div class="p-item">
                      <div>
                        <svg t="1656687602044" class="icon3" viewBox="0 0 1166 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6817" width="200" height="200">
                          <path
                            d="M882.058985 2.769416C863.503898 0.969296 845.641165 0 829.024669 0c-115.623117 0-184.166162 42.510535-246.201079 107.868752C512.341954 33.094521 431.751949-11.631547 283.588195 2.769416 158.964476 14.954846 17.170379 124.623718 0 336.345569v69.235399C13.84708 589.470188 142.486451 739.434063 507.495476 999.897634a129.331726 129.331726 0 0 0 150.379287 0c365.285966-261.017455 493.371454-410.981329 507.633946-594.870549v-69.235399C1148.476801 124.623718 1006.682703 14.954846 882.058985 2.769416z"
                            fill="#f65f9f"
                            p-id="6818"
                          ></path>
                        </svg>
                      </div>
                      <div>80</div>
                    </div>
                  </div>
                </div>
              </div>
              <v-md-editor :model-value="text" mode="preview" @copy-code-success="handleCopyCodeSuccess"></v-md-editor>
            </div>
            <!--        评论 -->
            <div class="comment">
              <Comment></Comment>
            </div>
          </div>
          <div class="right">
            <div class="right-main">
              <HotPost></HotPost>
              <HotComments></HotComments>
            </div>
          </div>
        </div>
      </div>
      <!--        底部 版权数据 -->
      <!--        bottom-->
      <Copyright></Copyright>
    </el-main>
  </el-container>
</template>

<script lang="ts" setup>
import MenuBoard from '../../../components/blog/MenuBoard.vue';
import Copyright from '../../../components/blog/Copyright.vue';
import HotPost from '../../../components/blog/HotPost.vue';
import HotComments from '../../../components/blog/HotComments.vue';
import Comment from '../../../components/blog/Comment.vue';

const img = ref('http://file.luanshiliunian.cn/11111_1637034086344.jpg');
const title = ref('');
const content = ref('夜深了，世界安静了，风不再呼喊，树也不再摇曳，更像是睡着了。是否你也和树做了一个同样的梦，向往着星辰大海，向往着盛世繁华。');
const date = ref('2021-11-26');
const look = ref(51);
const comment = ref(85);
const text = ref(`
## 全点反算-开始推演
> url
\`\`\`
/priDeduce/startAllPointDeduce
\`\`\`
> params-第一种情况
- 所选指标是   年缴保费、基本保险金额
\`\`\`json
{
  "id": 296,
  "priProductPricing": 1398,
  "baseDefinition": "",   //责任保持原有
  "baseAssum": "",//假设保持原有
  "baseIndex":  "",// 保持原有
  "result": null,// 保持原有
  "createUser": 1,// 保持原有
  "createDate": "2022-09-13",// 保持原有
  "isValid": 1,// 保持原有
  "modelFilepath": "",// 保持原有
  "productId": 1062, // 保持原有
  "rawBaseDefinition": "",// 保持原有
  "type": 1, // 保持原有

  // 如果 计算的指标是保险金额 就是"从产品选择"  弹窗 选择的指标  则传输次参数
  "singlePointList": "",
  // 如果 计算的指标是保险金额 就是"从产品选择"  弹窗 选择的指标  则传输次参数  如果当前选择的指标是 保险金额则传 "basicInsurance"  指标=年交保费 则传输 "yearPremium"
  "contrastProperty": "basicInsurance"
}
\`\`\`

> params-第二种情况
- 所选指标是   年缴保费、基本保险金额 除外的其他指标
\`\`\`json
{
  "id": 296,
  "priProductPricing": 1398,
  "baseDefinition": "",   //责任保持原有
  "baseAssum": "",//假设保持原有
  "baseIndex":  "",// 保持原有
  "result": null,// 保持原有
  "createUser": 1,// 保持原有
  "createDate": "2022-09-13",// 保持原有
  "isValid": 1,// 保持原有
  "modelFilepath": "",// 保持原有
  "productId": 1062, // 保持原有
  "rawBaseDefinition": "",// 保持原有
  "type": 1, // 保持原有
  "singlePointSetting": {   // 本参数需要吧json对象 转为字符串  下面为方便展示写出
    "age": [40, 39,38,37,36,35,34,33,32,31,30], //年龄
    "sex": [ "男性","女性"], // 性别
    "bTerm": [ "保至终身","保至88"], //保险期间
    "pTerm": [10, 9, 7, 6, 5], // 交费期间
    "basicInsurance": [ 500000.0, 300000.0], //基本保险金额  此字段为计算型字段 需要删除本字段留空  基本保险金额和年交保费这两个key只能存在一个
    "yearPremium": [ 5000, 4000], //年交保费 如果此字段为计算型字段 需要删除本字段留空
  }


}
\`\`\`



`);

const handleCopyCodeSuccess = (data: string) => {
  console.log(data);
};
</script>

<style scoped lang="scss">
.header {
  background-color: var(--bg-brand-color);
}

body {
  overflow-x: hidden;
  overflow-y: hidden;
}

.body {
  background-color: var(--bg-brand-color);
}

.main {
  margin-top: 20px;
}

.el-header {
  padding: 0 !important;
}

.el-main {
  padding: 0 !important;
}
</style>

<style lang="scss">
.label {
  display: flex;
  flex-wrap: wrap;

  .label-item {
    background: var(--bg-brand-color)-menu;
    padding: 4px 5px;
    margin-top: 10px;
    border-radius: 3px;
    margin-right: 5px;
    display: flex;
    align-items: center;
    color: #fff;
    font-size: 10px;
    font-weight: bold;

    svg {
      width: 12px;
      height: 12px;
      fill: #fff;
    }
  }

  //.label-item:nth-child(n) {
  //    background: @second-color;
  //}
  //
  //.label-item:nth-child(2n) {
  //    background: @angle-color;
  //}
  //
  //.label-item:nth-child(3n) {
  //    background: @high-color;
  //}

  .label-item:nth-child(4n) {
    background: var(--warning-color);
  }

  .label-item:nth-child(5n) {
    background: var(--bg-brand-color);
  }
}

.middle-t {
  display: flex;
  justify-content: space-between;
  align-items: center;

  .p-item {
    color: var(--text-color) 2;
    font-size: 10px;
    font-weight: bold;
    display: flex;
    align-items: center;
    padding: 5px 5px 5px 5px;

    background: var(--border-color);
    margin-right: 20px;
    min-width: 60px;
    border-top-right-radius: 10px;
    border-top-left-radius: 10px;
    border-bottom-left-radius: 10px;

    div {
      padding-left: 5px;
    }
  }
}

.comment-main-reply {
  display: flex;
  padding: 20px 0;

  .t-left img {
    width: 3.001rem;
    height: 3.001rem;
    border-radius: 150%;
  }

  .right-top-main {
    display: flex;
    justify-content: space-between;
    align-items: center;

    .right-top {
      display: flex;
      align-items: center;
      margin-bottom: 8px;
    }

    .right-top-name {
      padding-right: 10px;
      font-weight: bold;
      font-size: 1.18rem;
      color: var(--text-color);
    }

    .right-top-id {
      display: flex;
      align-items: center;
      background-color: #d29073;
      border-radius: 5px;
      padding: 1px 5px;

      .right-top-grade {
        font-size: 10px;
        color: #fff5d0;
      }
    }

    .right-top-tag {
      width: 15px;
      height: 15px;
    }
  }

  .right-info-time {
    color: var(--text-color) 2;
  }

  .t-right {
    display: flex;
    flex-direction: column;
    justify-content: center;
    padding-left: 0.8rem;
  }
}

.article-top {
  background: var(--el-bg-color);
  margin-bottom: 2px;
  padding: 30px 20px 30px 30px;
  border-radius: 8px 8px 0 0;

  .post-title {
    font-size: 1.5rem;
    line-height: 150%;
    font-weight: bolder;
    color: var(--text-color);
  }

  .property-s {
    display: flex;
    flex-wrap: wrap;
    margin-top: 10px;

    .property {
      color: var(--text-color) 2;
      font-size: 10px;
      font-weight: bold;
      display: flex;
      margin-right: 5px;
      //min-width: 130px;
      align-items: center;
      padding: 10px 10px 10px 0;
    }
  }
}

.vuepress-markdown-body {
  color: var(--text-color) !important;
  background-color: var(--el-bg-color) !important;
}
</style>

<style lang="scss" scoped>
.body {
  width: 100%;
  display: flex;
  justify-content: center;
  background: var(--bg-brand-color);
}

.v-md-editor {
  background-color: #00000000 !important;
}

.vuepress-markdown-body {
  background-color: #00000000 !important;
  border-radius: 5px !important;
}

.main {
  //width: 1300px;
  display: flex;
}

.left {
  margin: 10px;
  width: 900px;
}

.right {
  width: 400px;
}

.right-main {
  padding: 10px;
}

.left-main {
  //padding: 10px;
}

.comment {
  padding-top: 2px;
}

.icon3 {
  width: 20px;
  height: 20px;
}

@media screen and (max-width: 1400px) {
  .left {
    width: auto;
  }

  .right {
    width: 100%;
    /*display: none;*/
  }

  .main {
    flex-direction: column;
  }
}

@media screen and (max-width: 800px) {
  .middle-t {
    display: flex;
    flex-direction: column;
    justify-content: space-between;
    align-items: flex-start;
  }

  .middle-p {
    display: flex;
    align-items: flex-end;
  }
}
</style>
